<x-layout>
    ...
    <h2 class="text-2xl font-bold mb-4"> <span class="text-gray-300">Daftar Produk</span> Update Produk</h2>
    ...
    <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="flex flex-wrap gap-4 p-4">
                ...
                <div class="flex items-center border rounded px-3 py-2 bg-white">
                    <select class="w-full focus:outline-none ml-2 text-gray-400" name="kategori" id="kategori_id">
                        <option value="" disabled selected hidden>Pilih
                            Kategori</option>
                        <option value="alat_olahraga"
                            {{ $product->kategori_produk == 'Alat Olahraga' ? 'selected' : '' }}>Alat Olahraga</option>
                        <option value="alat_musik" {{ $product->kategori_produk == 'Alat Musik' ? 'selected' : '' }}>
                            Alat Musik</option>
                    </select>
                </div>
            </div>
            ...
            <div class="w-[682px]">
                ...
                <div class="flex items-center border raounded px-3 py-2 bg-white">
                    <input name="nama_barang" value="{{ $product->nama_produk }}" required
                        class="w-full focus:outline-none ml-2" type="text" placeholder="Masukan Nama Barang">
                </div>
            </div>
        ...
        <div class="flex flex-wrap gap-4 p-4">
            <div class="w-[300px]">
                ...
                <div class="flex items-center border rounded px-3 py-2 bg-white">
                    <input name="harga_beli" value="{{ $product->harga_beli }}" required id="harga_beli" type="number"
                        class="w-full focus:outline-none ml-2" placeholder="Masukan Harga Beli">
                </div>
            </div>
            <div class="w-[320px]">
                <label for="" class="block text-gray-700 font-semibold mb-1">Harga Jual</label>
                <div class="flex items-center border rounded px-3 py-2 bg-white">
                    <input name="harga_jual" value="{{ $product->harga_jual }}" required type="number" id="harga_jual"
                        class="w-full focus:outline-none ml-2" placeholder="Masukan Harga Jual" readonly>
                </div>
            </div>
            ...
            <div class="w-[348px]">
                ...
                <div class="flex items-center border rounded px-3 py-2 bg-white">
                    <input name="stok" value="{{ $product->stok }}" required type="number"
                        class="w-full focus:outline-none ml-2" placeholder="Masukan stok barang">
                </div>
            </div>
            ...
            <div class="w-[1000px]">
                ...
                <div
                    ...
                    <label class="cursor-pointer items-center flex-col  text-blue-600 px-4 py-2">
                        ...
                        <input name="image" value="{{ $product->image }}" required type="file" id="uploadInput"
